...
### Rational valuation and acquisition evidence

**Established principles.** Systems establish constraints/opportunities; actors determine marginal value from match state. Do not enforce good team play; construct an economy in which rational good team play tends to be efficient. A resource may be systemically critical and still have low marginal universal value. Model demand as an input to actor policy, not an explicit current-relevance modifier on base XP.

Extraction means search/identify → reach/access → expose → mine/acquire: efficient conversion of difficult-to-access world resources into acquired material. Keep intrinsic acquisition challenge, generated geographic evidence and player-contingent realized cost separate. Material market/use value and Extraction task significance are different observables. Attainment XP need not wait for downstream use or delivery.

**Working future interface.** Store intrinsic acquisition profiles alongside references to worldgen Opportunity Relationships, not a parallel spatial score. Reuse Reach/Payoff, arrival envelopes, local continuation, deep network reach and all five `commitment_profile` dimensions: `access_burden`, `exploitation_burden`, `return_burden`, `infrastructure_dependency`, `exposure`. Record evidence source, generation snapshot, validation status and unresolved fields. Avoid double counting intrinsic exploitation work already described by commitment evidence.

The current shadow analysis from commit `3f174caf89431c612bf2709ab5c8573e3443489f` is diagnostic only. Its evidence MUST NOT become hard XP formulas, weights, quotas or generation gates. Exposure is unmeasured; resource abundance, ore yield and ecological payoff are not established by terrain masks. Eventual generated progression significance should combine intrinsic challenge with validated access/commitment evidence only after a further design and validation decision.

Infrastructure can reduce realized acquisition cost without reducing underlying generated progression significance: faster meaningful acquisition rewards efficiency as XP/hour. Player-specific inconvenience, current demand and detours remain actor-policy inputs. Preserve existing integration mechanics separately. No default repeat penalties are proposed: test whether XP requirements, renewal intervals, search/travel, inventory and opportunity cost already discourage repeated low-value chasing.

### Renewable-resource matrix — Working schema

This is a design worksheet direction, not a balance table or generator contract. Each resource/ecology row should carry status and provenance; unknown values remain Open. Separate the persistent opportunity host from each availability episode/current manifestation.

| Field | Meaning |
|---|---|
| Economic class | Staple, founder stock, consumable, construction feedstock, processing input or other economic role; roles can overlap |
| Systemic dependencies / importance | Systems relying on recurring access, and consequences of absence |
| Marginal universality | How broadly an additional unit is useful across actors and match states, distinct from systemic importance |
| Founder value | Value as propagation stock or a means to establish future production |
| Immediate payload | Direct acquired items and possible immediate use, before processing |
| Processing uplift | Added uses/value from transforming inputs; preserve input/output accounting |
| Renewal object | Geography, ecology or opportunity relationship restored; distinguish its current manifestation |
| Recurrence importance | Why reliable future access matters |
| Occurrence frequency | How often supported opportunities occur at a declared spatial/time unit |
| Manifestation scale | Size, spread and material/population payload of an availability episode |
| Spatial ecology / opportunity host | Supported terrain/ecology and links to Handoff/Affordance/Reach/Payoff evidence |
| Depth eligibility | Working eligibility/vocabulary hypothesis and provenance, not a universal distance tier |
| Active occupancy | Simultaneously available opportunities at a declared spatial unit and time |
| XP attraction | Expected progression incentive relative to alternative activity; no selected reward yet |
| Expected relevance | Actor- and state-dependent reasons to collect, ignore, cache or revisit; not an XP modifier |
| Intrinsic acquisition profile | Concealment, identification, geometry, dispersion, tool requirements, excavation and intrinsic hazards |

Initial qualitative rows to investigate:

| Working candidate | Economic distinction to preserve | Renewal host to investigate | Unsettled decision |
|---|---|---|---|
| Crops / livestock | Food, propagation and specialized outputs; wool can be important to explorer banners without every actor wanting more | Crop ecology / managed or wild population opportunities | Species membership, founder access, population rules and depth |
| Carrots / potatoes | Immediate food versus planting stock; processing uplift differs | Crop-bearing geography with current patches | Founder scarcity, staple status and depth; no fixed counts |
| Trees | Construction and processing inputs; species/ecological identity | Tree ecology, not identical replacement logs | Eligible ecology, disturbance and management rules |
| Copper / iron / coal | Infrastructure, equipment and processing dependencies; copper's chest/Supply Line relevance does not imply universal marginal demand | Geologically supported deposit opportunities | Membership, recurrence, manifestation and acquisition profiles |
| Clay / gravel / sand | Important construction/processing feedstocks can have low marginal universal value | Coherent resource-bearing geography | Membership and saturation; easy exposed material need not attract large Extraction XP |
| Diamond | High-power output may warrant finite/restricted access | Finite or restricted geology candidate | No new renewal entitlement |

These rows begin the matrix; omitted cell values are Open, not inferred defaults. Existing canon still governs resource functionality. More resources may qualify where systemic importance warrants recurring access, but that does not imply every vanilla-renewable resource qualifies or every important resource should recur.

Renewable abundance means reliable future access rather than many simultaneous visible manifestations. Track active opportunity saturation independently of recurrence reliability, candidate density and eligible share. Authored regenerative content may complete coherent geographic relationships; it must not manufacture payoff in arbitrary terrain. Renewal restores opportunity, not exact blocks, passive income or guaranteed throughput.
...
The current nighttime contracts in maps/objectives remain in force; the existing experiment's day/night language does not reopen established Mob Swarm behavior. Current Capacity terminology and infrastructure contracts supersede historical Flow Weight discussion. Existing Hunger cadence and class decisions are unchanged. This model supplies no numerical XP calibration.

## OPEN QUESTIONS

- What evidence validates generated acquisition significance, and how are overlapping intrinsic/worldgen burdens attributed without double counting? No shadow-to-XP mapping is selected.
- What opportunity-level attainment event earns credit, how is partial/shared work attributed, and how does it interact with Exploration, Development and class-specific resource acquisition?
- How should changed terrain, permanent access improvements and renewal episodes retain generated significance while distinguishing newly created opportunities? This does not settle dynamic eligibility.
- Which matrix memberships, founder roles, depth eligibility, recurrence and active-occupancy ranges support meaningful choices? What observed behavior would justify repeat restrictions, if any?
