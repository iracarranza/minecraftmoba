...
import argparse, json, math, random, statistics, zipfile
...
def cells_from_export(opp, predict):
    ...
    for r in opp["cells"]:
        ...
        out.append({
            ...
            # Count-calibrated proxies only; not claimed as original classifier.
...
def metrics(cfg):
    def side_sum(items,team,key):
        return sum(x[key] for x in items if x["bias"]==team)
    fn,fs=side_sum(cfg["founders"],"north","n"),side_sum(cfg["founders"],"south","s")
    rn,rs=side_sum(cfg["renewables"],"north","n"),side_sum(cfg["renewables"],"south","s")
    wn=statistics.mean(x["n"] for x in cfg["worksites"])
    ws=statistics.mean(x["s"] for x in cfg["worksites"])
    pn=sum(x["n"] for x in cfg["pois"]); ps=sum(x["s"] for x in cfg["pois"])
    rtn=statistics.mean(x["n"] for x in cfg["route_targets"]["north"])
    rts=statistics.mean(x["s"] for x in cfg["route_targets"]["south"])
    comps={"founder":ndiff(fn,fs),"renewable":ndiff(rn,rs),
           "worksite":ndiff(wn,ws),"poi":ndiff(pn,ps),"route":ndiff(rtn,rts)}
    balance=sum(comps.values())/len(comps)
    ...
    return {"balance_asymmetry":balance,"components":comps,
            "contest_pressure":contest,"exploration_pressure":exploration,
            "consolidation_pressure":consolidation,"resource_density":density,
            "peripheral_specialization":peripheral}

def objective(profile,m):
    b=m["balance_asymmetry"]
    if profile=="exploration_centric": char=m["exploration_pressure"]
    elif profile=="consolidative": char=m["consolidation_pressure"]
    elif profile=="resource_dense": char=m["resource_density"]
    elif profile=="resource_light": char=-m["resource_density"]+.5*m["exploration_pressure"]
    elif profile=="contested_core": char=1.4*m["contest_pressure"]
    elif profile=="peripheral_specialization": char=m["peripheral_specialization"]
    elif profile=="route_centric": char=.5*m["exploration_pressure"]+.3*m["contest_pressure"]
    else:
...
def main():
    ...
    (a.out/"candidate_catalog.json").write_text(json.dumps(cats,indent=2))
    (a.out/"scenario_frontier.json").write_text(json.dumps(frontier,indent=2))
    (a.out/"unresolved_hazards.json").write_text(json.dumps(hazard,indent=2))
    print(json.dumps({p:{"balanced_found":v["balanced_found"],
                         "best_balance":v["finalists"][0]["metrics"]["balance_asymmetry"]}
                      for p,v in frontier.items()},indent=2))
