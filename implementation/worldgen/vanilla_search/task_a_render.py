...
def svg(fit, candidate, background):
    ...
    refined=fit.get('central_connective_area')
    if refined:
        x0,x1,z0,z1=refined['logical_bounds']
        parts.append(f'<rect x="{ox+x0*scale}" y="{oy+z0*scale}" width="{(x1-x0+1)*scale}" height="{(z1-z0+1)*scale}" stroke="#ffec94" stroke-dasharray="8 5" fill="none"/>')
        mask=unrle(refined['traversable_mask_rle'])
        # Row runs keep the connected-area overlay compact and readable.
        for z in range(h):
            start=None
            for x in range(w+1):
                active=x<w and mask[z*w+x]
                if active and start is None: start=x
                if not active and start is not None:
                    parts.append(f'<rect x="{ox+start*scale}" y="{oy+z*scale}" width="{(x-start)*scale}" height="{scale}" fill="#ffec94" fill-opacity=".18"/>')
                    start=None
        label([x0,z0],'CONNECTIVE INTERIOR','#ffec94')
    ...
    if center and center['reference'] and not refined:
    ...
    for offset,team in enumerate(('north','south')):
        ...
        for row,(name,band) in enumerate((('secondary','70–110'),('tertiary','150–210'),('deep','250–350+'))):
            d=access.get(name,{})
            if refined:
                d=fit['operational_depth'][team]['bands'][name]
